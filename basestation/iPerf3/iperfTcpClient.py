...
if result.error:
	...
	print(data)
	cur.execute("INSERT INTO iperf(protocol,txBytes,jitter,avgCpuLoad,MB_s,mbps,deviceIp) VALUES(%(protocol)s,%(txBytes)s,%(jitter)s,%(avgCpuLoad)s,%(MB_s)s,%(mbps)s,%(deviceIp)s);",data)
	conn.commit()
	conn.close()
